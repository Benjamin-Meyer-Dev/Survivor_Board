...
export function renderDrive(root, board, viewWeek, onSelectWeek) {
  ...
  root.querySelectorAll("[data-week]").forEach((row) => {
    row.addEventListener("click", () => onSelectWeek(Number(row.dataset.week)));
  });
}

function rowMarkup(week, board, viewWeek) {
  ...
  return `
    <button type="button" class="${classes}" data-week="${week.week}" data-motion-key="drive-${week.week}"
      ...
      <span class="drive__wk">${week.week}${week.week === board.currentWeek && !board.eliminated ? BALL : ""}</span>
...
function statusChip(week, kind, moot) {
  const results = week.picks.map((pick) => pick.status.result).filter(Boolean);
  if (results.includes("L")) return '<span class="chip chip--danger">Lost</span>';
  if (results.length) return '<span class="chip chip--safe">Won</span>';
  if (moot) return "";
